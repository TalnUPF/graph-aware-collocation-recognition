...
if __name__ == "__main__":
    ...
    out_slot_label_list = json.load(
        open(os.path.join(results_path, "out_slot_label_list_%s_1.json" % (results_mode)), "r"))
    slot_preds_list = json.load(open(os.path.join(results_path, "slot_preds_list_%s_1.json" % (results_mode)), "r"))
    with open(os.path.join(results_path, "classification_report_%s_1.txt" % (results_mode)), "w") as fout:
